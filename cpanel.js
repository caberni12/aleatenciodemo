// ALE ATENCIO R9.18.85 · carrito sobre header + URL Eventos configurable
...
function creditIsOverdue(c){if(!c?.fecha_vencimiento)return false;const due=new Date(`${String(c.fecha_vencimiento).slice(0,10)}T23:59:59`);return !Number.isNaN(due.getTime())&&due.getTime()<Date.now()}
...
function renderWholesaleCreditClientSummary(clientId,targetId){const host=$(targetId);if(!host)return;const c=wholesaleCreditByClient(clientId),client=wholesaleClientById(clientId);if(!clientId||!client){host.innerHTML='Selecciona un Mayorista para ver su situación actual.';return}if(!c){host.innerHTML=`<strong>${esc(client.razon_social||client.nombre||'Mayorista')}</strong><span>Sin línea de crédito asignada.</span>`;return}host.innerHTML=`<strong>${esc(client.razon_social||client.nombre||'Mayorista')}</strong><span>Límite: ${money(c.limite_credito)} · Disponible: ${money(c.saldo_disponible)} · Utilizado: ${money(c.saldo_utilizado)} · Vence: ${esc(creditDateLabel(c.fecha_vencimiento))}</span>`}
function renderWholesaleCredits(){
  fillWholesaleSelectors();
  const credits=data.wholesaleCredits||[],moves=data.wholesaleCreditMovements||[],clients=data.wholesalers||[];
  ...
  if($("#wholesaleCreditsTable"))$("#wholesaleCreditsTable").innerHTML=table(["Mayorista","Límite","Disponible","Utilizado","Asignación","Vencimiento","Estado","Acciones"],credits.map(c=>{const client=clients.find(x=>String(x.id)===String(c.cliente_id)),status=!wholesaleActive(c.activo)?'Suspendido':creditIsOverdue(c)?'Vencido':'Activo';return `<tr><td><strong>${esc(client?.razon_social||client?.nombre||c.cliente_id||'')}</strong><br><small>${esc(client?.rut||'')}</small></td><td>${money(c.limite_credito)}</td><td><strong>${money(c.saldo_disponible)}</strong></td><td>${money(c.saldo_utilizado)}</td><td>${esc(creditDateLabel(c.fecha_asignacion))}</td><td>${esc(creditDateLabel(c.fecha_vencimiento))}</td><td><span class="role-badge ${status==='Activo'?'':'inactive-badge'}">${esc(status)}</span></td><td><div class="row-actions"><button class="btn btn-light btn-compact" onclick="editWholesaleCredit('${esc(c.cliente_id)}')">Editar</button><button class="btn btn-light btn-compact" onclick="prepareWholesaleCreditPayment('${esc(c.cliente_id)}')">Abono</button></div></td></tr>`}).join(""));
  if($("#wholesaleCreditMovementsTable"))$("#wholesaleCreditMovementsTable").innerHTML=table(["Fecha","Mayorista","Movimiento","Monto","Disponible","Utilizado","Referencia"],moves.map(m=>{const client=clients.find(x=>String(x.id)===String(m.cliente_id));return `<tr><td>${esc(formatDate(m.creado_en))}</td><td>${esc(client?.razon_social||client?.nombre||m.cliente_id||'')}</td><td><span class="role-badge">${esc(m.tipo||'')}</span></td><td>${money(m.monto)}</td><td>${money(m.saldo_disponible_resultante)}</td><td>${money(m.saldo_utilizado_resultante)}</td><td>${esc(m.referencia||m.detalle||'—')}</td></tr>`}).join(""));
}
function clearWholesaleCreditForm(){if($("#whCreditClient"))$("#whCreditClient").value="";if($("#whCreditLimit"))$("#whCreditLimit").value="";if($("#whCreditAssigned"))$("#whCreditAssigned").value=new Date().toISOString().slice(0,10);if($("#whCreditDue"))$("#whCreditDue").value="";if($("#whCreditActive"))$("#whCreditActive").value="SI";if($("#whCreditNotes"))$("#whCreditNotes").value="";renderWholesaleCreditClientSummary("","#whCreditLiveSummary")}
window.editWholesaleCredit=id=>{const c=wholesaleCreditByClient(id);$("#whCreditClient").value=id;$("#whCreditLimit").value=c?formatClpEditable(c.limite_credito):"0";$("#whCreditAssigned").value=c?.fecha_asignacion?String(c.fecha_asignacion).slice(0,10):new Date().toISOString().slice(0,10);$("#whCreditDue").value=c?.fecha_vencimiento?String(c.fecha_vencimiento).slice(0,10):"";$("#whCreditActive").value=wholesaleActive(c?.activo??true)?"SI":"NO";$("#whCreditNotes").value=c?.observaciones||"";renderWholesaleCreditClientSummary(id,"#whCreditLiveSummary");openAdminView("wholesale-credits");window.scrollTo({top:0,behavior:"smooth"})}
window.prepareWholesaleCreditPayment=id=>{$("#whCreditPaymentClient").value=id;renderWholesaleCreditClientSummary(id,"#whCreditPaymentSummary");openAdminView("wholesale-credits");requestAnimationFrame(()=>$("#whCreditPaymentAmount")?.focus())}
$("#whCreditClient")?.addEventListener("change",e=>{const id=e.currentTarget.value,c=wholesaleCreditByClient(id);if(c){$("#whCreditLimit").value=formatClpEditable(c.limite_credito);$("#whCreditAssigned").value=String(c.fecha_asignacion||"").slice(0,10);$("#whCreditDue").value=String(c.fecha_vencimiento||"").slice(0,10);$("#whCreditActive").value=wholesaleActive(c.activo)?"SI":"NO";$("#whCreditNotes").value=c.observaciones||""}else{$("#whCreditLimit").value="";$("#whCreditAssigned").value=new Date().toISOString().slice(0,10);$("#whCreditDue").value="";$("#whCreditActive").value="SI";$("#whCreditNotes").value=""}renderWholesaleCreditClientSummary(id,"#whCreditLiveSummary")});
...
$("#saveWholesaleCredit")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const cliente_id=$("#whCreditClient").value;if(!cliente_id)return toast("Selecciona un Mayorista");const limite_credito=parseClpAmount($("#whCreditLimit").value);if(limite_credito<0)return toast("El límite no puede ser negativo");const fecha_asignacion=$("#whCreditAssigned").value||new Date().toISOString().slice(0,10),fecha_vencimiento=$("#whCreditDue").value;if(!fecha_vencimiento)return toast("Define la fecha tope de pago");if(fecha_vencimiento<fecha_asignacion)return toast("La fecha tope debe ser igual o posterior a la asignación");await AleAPI.post("adminwholesalecreditsave",{cliente_id,limite_credito,fecha_asignacion,fecha_vencimiento,activo:$("#whCreditActive").value,observaciones:$("#whCreditNotes").value.trim()},token);toast("✓ Línea de crédito actualizada y Mayorista notificado");await loadAdminModules({modules:["wholesale"],retry:true});renderWholesaleCredits()}catch(err){console.warn(err);const c=String(err?.message||err||"").toUpperCase();toast(c.includes("FECHAS_CREDITO_INVALIDAS")?"Revisa las fechas de la línea de crédito":`✕ No fue posible guardar la línea · ${c}`)}}));
$("#registerWholesaleCreditPayment")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const cliente_id=$("#whCreditPaymentClient").value,monto=parseClpAmount($("#whCreditPaymentAmount").value);if(!cliente_id)return toast("Selecciona un Mayorista");if(!(monto>0))return toast("Ingresa un monto válido");await AleAPI.post("adminwholesalecreditpayment",{cliente_id,monto,referencia:$("#whCreditPaymentRef").value.trim()},token);toast("✓ Abono registrado · saldo de crédito liberado");$("#whCreditPaymentAmount").value="";$("#whCreditPaymentRef").value="";await loadAdminModules({modules:["wholesale"],retry:true});renderWholesaleCredits();renderWholesaleCreditClientSummary(cliente_id,"#whCreditPaymentSummary")}catch(err){console.warn(err);const c=String(err?.message||err||"").toUpperCase();toast(c.includes("CREDITO_SIN_DEUDA")?"Ese Mayorista no tiene deuda pendiente":c.includes("CREDITO_NO_ASIGNADO")?"Ese Mayorista no tiene línea de crédito":`✕ No fue posible registrar el abono · ${c}`)}}));
$("#refreshWholesaleCredits")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{await loadAdminModules({modules:["wholesale"],retry:true});renderWholesaleCredits()},"Actualizando…"));
