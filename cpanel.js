...
function renderOrderDetail(order,items,history=[]){
  ...
  const proofBox=$("#orderTransferProof"),proofLink=$("#orderTransferProofLink"),proofState=$("#orderTransferProofState"),approveBtn=$("#approveTransferPayment"),payWa=$("#sendOrderPaymentWhatsApp"),cancelBtn=$("#cancelOrderBtn"),regenBtn=$("#regenerateOrderPdf");if(proofBox){const transfer=String(order.medio_pago||"").toUpperCase()==="TRANSFERENCIA";proofBox.classList.toggle("hidden",!transfer);if(transfer){const has=!!order.comprobante_pago_url;proofLink.classList.toggle("hidden",!has);if(has)proofLink.href=order.comprobante_pago_url;proofState.textContent=has?(order.comprobante_pago_estado||"PENDIENTE_REVISION"):"Aún sin comprobante";approveBtn.classList.toggle("hidden",final||!has||String(order.estado_pago||"").toUpperCase()==="PAGADO")}}if(payWa)payWa.classList.toggle("hidden",final||String(order.medio_pago||"").toUpperCase()!=="TRANSBANK"||String(order.estado_pago||"").toUpperCase()==="PAGADO");if(cancelBtn)cancelBtn.classList.toggle("hidden",final);if(regenBtn){regenBtn.disabled=final;regenBtn.title=final?orderFinalMessage(finalState):"";}
