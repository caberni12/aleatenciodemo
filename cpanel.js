...
function renderOrders(){
  ...
  host.innerHTML=table(["N.º pedido","Fecha","Cliente / RUT","Contacto","Entrega","Total","Pago","Estado","PDF","Acciones"],data.orders.map(o=>`<tr>
    ...
    <td><span class="payment-status-badge payment-${esc(String(o.estado_pago||"PENDIENTE").toLowerCase().replace(/[^a-z0-9]+/g,"-"))}">${esc(o.estado_pago||"PENDIENTE")}</span>${o.medio_pago?`<br><small>${esc(o.medio_pago)}</small>`:""}</td>
...
async function refreshTransbankHealth(showToast=false){
  const status=$("#transbankStatus"),env=$("#transbankEnvironment"),creds=$("#transbankCredentials");
  try{
    ...
    if(status){status.textContent=out.ready?"Listo para cobrar":out.url_ready?"Faltan credenciales":"Falta URL";status.classList.toggle("is-ready",!!out.ready);status.classList.toggle("is-warning",!out.ready)}
    if(showToast)toast(out.ready?"✓ Transbank está listo para crear transacciones":"Configuración incompleta: revisa URL y Secrets del servidor");
    return out;
  }catch(err){console.warn("transbankhealth",err);if(status){status.textContent="No verificado";status.classList.remove("is-ready");status.classList.add("is-warning")}if(creds)creds.textContent="No verificado";if(showToast)toast("✕ No fue posible verificar Transbank");return null}
}
function renderPayments(){const c=data.config||{},url=c.transbank_return_url||"";if($("#pTransbankReturnUrl"))$("#pTransbankReturnUrl").value=url;refreshTransbankHealth(false)}
$("#savePayments")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const raw=$("#pTransbankReturnUrl").value.trim(),url=raw?safeHttpsAdminUrl(raw):"",enabled=!!url;await AleAPI.post("saveConfig",{transbank_enabled:enabled?"SI":"NO",transbank_return_url:url,transbank_button_label:"Pagar con Transbank"},token);toast(enabled?"✓ URL guardada. Verificando Transbank…":"✓ Transbank quedó desactivado");await reload();await refreshTransbankHealth(false)}catch(err){console.warn(err);const code=String(err?.message||"");toast(code.includes("TRANSBANK_URL_RETORNO")||code.includes("URL_HTTPS")?"✕ La URL debe ser HTTPS, por ejemplo https://tu-dominio.cl/":"✕ No fue posible guardar Transbank")}}));
