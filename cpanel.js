...
const siiState={config:null,certificados:[],caf:[],documentos:[],intercambios:[],intercambioRespuestas:[],certificacion:[],certMetricas:{},loaded:false};
...
function renderSiiExchange(){const tbody=$("#siiExchangeTable");if(!tbody)return;const rows=siiState.intercambios||[];tbody.innerHTML=rows.length?rows.map(x=>{const firma=x.firma_valida===true?'<span class="sii-status-pill ok">Válida</span>':x.firma_valida===false?'<span class="sii-status-pill error">No válida</span>':x.firma_presente?'<span class="sii-status-pill warn">No verificada</span>':'<span class="sii-status-pill warn">Sin firma</span>';return `<tr><td>${esc(siiFormatDate(x.fecha_emision||x.recibido_en))}</td><td><strong>${esc(x.rut_emisor||'')}</strong></td><td><strong>${Number(x.tipo_dte)} / ${Number(x.folio).toLocaleString('es-CL')}</strong><small>${esc(SII_DTE_NAMES[x.tipo_dte]||'DTE')}</small></td><td class="money-cell">${money(x.monto_total||0)}</td><td>${firma}</td><td><span class="sii-status-pill ${String(x.estado_recepcion).includes('CONFORME')?'ok':'warn'}">${esc(x.estado_recepcion||'PENDIENTE')}</span></td><td><span class="sii-status-pill ${x.estado_comercial==='ACEPTADO'?'ok':x.estado_comercial==='RECHAZADO'?'error':'warn'}">${esc(x.estado_comercial||'PENDIENTE')}</span>${x.acuse_mercaderias?'<small>Acuse mercaderías emitido</small>':''}</td><td><div class="sii-action-row"><button class="btn btn-light btn-compact" data-sii-x-response="RECEPCION_ENVIO" data-id="${esc(x.id)}">Recepción</button><button class="btn btn-light btn-compact" data-sii-x-response="ACEPTACION" data-id="${esc(x.id)}">Aceptar</button><button class="btn btn-light btn-compact" data-sii-x-response="RECHAZO" data-id="${esc(x.id)}">Rechazar</button><button class="btn btn-light btn-compact" data-sii-x-response="ACUSE_MERCADERIAS" data-id="${esc(x.id)}">Acuse</button><button class="btn btn-light btn-compact" data-sii-x-detail="${esc(x.id)}">XML</button></div></td></tr>`}).join(''):'<tr><td colspan="8" class="sii-document-empty">No hay DTE recibidos. Importa un EnvioDTE XML del proveedor o del set de intercambio del SII.</td></tr>'}
async function loadSiiExchange(){const out=await SiiAPI.exchangeList(token);siiState.intercambios=out.documentos||[];siiState.intercambioRespuestas=out.respuestas||[];renderSiiExchange();return out}
function renderSiiCertification(){const tbody=$("#siiCertificationTable");if(!tbody)return;const stages=siiState.certificacion||[],states=['PENDIENTE','EN_PROCESO','COMPLETADO','OBSERVADO'];tbody.innerHTML=stages.length?stages.map(x=>`<tr><td><strong>${Number(x.orden)}. ${esc(x.nombre||x.codigo)}</strong><small>${esc(x.codigo)}</small></td><td><select class="sii-cert-state" data-code="${esc(x.codigo)}">${states.map(s=>`<option value="${s}" ${s===x.estado?'selected':''}>${s.replace('_',' ')}</option>`).join('')}</select></td><td><input class="sii-cert-detail" data-code="${esc(x.codigo)}" value="${esc(x.detalle||'')}" placeholder="Resultado, TrackID, fecha o observación"><input class="sii-cert-evidence" data-code="${esc(x.codigo)}" value="${esc(x.evidencia||'')}" placeholder="Evidencia / referencia oficial"></td><td><button class="btn btn-primary btn-compact" data-sii-cert-save="${esc(x.codigo)}">Guardar</button></td></tr>`).join(''):'<tr><td colspan="4" class="sii-document-empty">Ejecuta el SQL R9.18.126 para habilitar el control de certificación.</td></tr>';$("#siiCertReceivedCount").textContent=Number(siiState.certMetricas?.dte_recibidos||0).toLocaleString('es-CL');$("#siiCertResponseCount").textContent=Number(siiState.certMetricas?.respuestas_generadas||0).toLocaleString('es-CL')}
