...
function renderPayments(){const c=data.config||{},enabled=yesNo(c.transbank_enabled)==="SI",url=c.transbank_payment_url||"";if($("#pTransbankUrl"))$("#pTransbankUrl").value=url;if($("#pTransbankLabel"))$("#pTransbankLabel").value=c.transbank_button_label||"Pagar con Transbank";const ready=enabled&&!!url,status=$("#transbankStatus");if(status){status.textContent=ready?"Listo para cobrar":"Sin configurar";status.classList.toggle("is-ready",ready)}}
$("#savePayments")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const raw=$("#pTransbankUrl").value.trim(),url=raw?safeHttpsAdminUrl(raw):"",label=$("#pTransbankLabel").value.trim()||"Pagar con Transbank",enabled=!!url;await AleAPI.post("saveConfig",{transbank_enabled:enabled?"SI":"NO",transbank_payment_url:url,transbank_button_label:label.slice(0,80)},token);toast(enabled?"✓ Transbank quedó activo en el carrito":"✓ Transbank quedó desactivado");await reload()}catch(err){console.warn(err);toast(String(err?.message||"").includes("URL_HTTPS")?"✕ La URL de Transbank debe comenzar con https://":"✕ No fue posible guardar Transbank")}}));
$("#testTransbankLink")?.addEventListener("click",()=>{try{const url=safeHttpsAdminUrl($("#pTransbankUrl").value);if(!url)return toast("Pega primero la URL de Transbank");window.open(url,"_blank","noopener")}catch(_){toast("✕ La URL debe comenzar con https://")}});
