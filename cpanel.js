...
const CPANEL_MEDIA_VERSION="20260922-r91858-modal-mayorista-fix";
...
function renderProducts(){
  ...
  const headers=canDelete?[`<span class="bulk-select-col">${bulkHeaderCheckbox("products",visibleIds)}</span>`,"Imagen","Producto","Categoría","Tamaños / precios","Stock","Estado","Destacado","Acciones"]:["Imagen","Producto","Categoría","Tamaños / precios","Stock","Estado","Destacado","Acciones"];
  const rows=list.map(p=>{const active=String(p.activo??"SI").toUpperCase()!=="NO",selected=selectedSet("products").has(String(p.id)),sizes=Array.isArray(p.tamanos)?p.tamanos.filter(x=>String(x.activo??"SI").toUpperCase()!=="NO"):[],sizeHtml=sizes.length?`<div class="product-size-summary">${sizes.map(z=>`<span><b>${esc(z.nombre)}</b> ${Number(z.precio||0)>0?money(z.precio):"Consultar"}</span>`).join("")}</div>`:`<span class="muted">Sin tamaños configurados</span>`,stock=sizes.length?sizes.reduce((a,z)=>a+Number(z.stock||0),0):toNumber(p.stock);return `<tr class="${active?"":"product-row-inactive"} ${selected?"is-selected":""}">${canDelete?`<td class="bulk-select-col">${bulkCheckbox("products",p.id)}</td>`:""}<td>${imgTag(p.image_url)}</td><td><strong>${esc(p.nombre)}</strong><br><small>${esc(p.descripcion||"")}</small></td><td>${esc(p.categoria_nombre||"")}</td><td>${sizeHtml}</td><td>${stock}</td><td><span class="product-state-badge ${active?"is-active":"is-inactive"}"><span class="product-state-dot" aria-hidden="true"></span>${active?"Activo":"Inactivo"}</span></td><td>${String(p.destacado).toUpperCase()==="SI"?"Sí":"No"}</td><td><div class="row-actions"><button type="button" data-edit-product="${esc(p.id)}">Editar tamaños</button>${canDelete?`<button type="button" class="danger" data-delete-product="${esc(p.id)}">Eliminar</button>`:""}</div></td></tr>`}).join("");
...
// ========================= R9.18.53 MAYORISTAS / LISTAS DE PRECIOS =========================
function renderWholesale(){
  const lists=data.priceLists||[],clients=data.wholesalers||[],users=data.wholesaleUsers||[],docs=data.wholesaleDocuments||[];fillWholesaleSelectors();
  ...
  if($("#priceListsTable"))$("#priceListsTable").innerHTML=table(["Lista","Código","Estado","Precios","Acciones"],lists.map(l=>{const count=(data.priceListItems||[]).filter(x=>String(x.lista_id)===String(l.id)&&wholesaleActive(x.activo)).length;return `<tr><td><strong>${esc(l.nombre||"")}</strong><br><small>${esc(l.descripcion||"")}</small></td><td>${esc(l.codigo||"")}</td><td><span class="role-badge ${wholesaleActive(l.activo)?"":"inactive-badge"}">${wholesaleActive(l.activo)?"Activa":"Inactiva"}</span></td><td>${count}</td><td><button class="btn btn-light btn-compact" onclick="editPriceList('${esc(l.id)}')">Editar precios</button></td></tr>`}).join(""));
...
function openPriceListEditor(id=""){
  const l=id?(data.priceLists||[]).find(x=>String(x.id)===String(id)):null;$("#plId").value=l?.id||"";$("#plName").value=l?.nombre||"";$("#plCode").value=l?.codigo||"";$("#plDescription").value=l?.descripcion||"";$("#plActive").value=wholesaleActive(l?.activo??true)?"SI":"NO";$("#priceListEditorTitle").textContent=l?`Editar ${l.nombre}`:"Nueva lista";const map=currentPriceListItems(l?.id||"");const rows=[];for(const p of data.products||[]){for(const z of (p.tamanos||[]).filter(x=>wholesaleActive(x.activo))){const it=map.get(`${p.id}|${z.id}`),value=it!==undefined?Number(it.precio||0):"";rows.push(`<tr data-product-id="${esc(p.id)}" data-size-id="${esc(z.id)}"><td><strong>${esc(p.nombre)}</strong></td><td>${esc(z.nombre)}</td><td>${money(z.precio||0)}</td><td><input class="plItemPrice" type="text" inputmode="numeric" value="${value!==""?esc(new Intl.NumberFormat('es-CL',{maximumFractionDigits:0}).format(value)):""}" placeholder="Sin autorizar"></td></tr>`)} }$("#priceListItemsEditor").innerHTML=table(["Producto","Tamaño","Precio normal","Precio lista"],rows.join(""));$("#priceListEditor").classList.remove("hidden");
}
window.editPriceList=id=>openPriceListEditor(id);$("#newPriceList")?.addEventListener("click",()=>openPriceListEditor());function closePriceListEditor(){$("#priceListEditor")?.classList.add("hidden")}$("#closePriceListEditor")?.addEventListener("click",closePriceListEditor);$("#cancelPriceList")?.addEventListener("click",closePriceListEditor);
$("#savePriceList")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const name=$("#plName").value.trim();if(!name)throw new Error("LISTA_NOMBRE_REQUERIDO");const items=$$("#priceListItemsEditor tr[data-product-id]").map(tr=>{const raw=tr.querySelector('.plItemPrice')?.value.trim()||"";if(!raw)return null;return{producto_id:tr.dataset.productId,tamano_id:tr.dataset.sizeId,precio:parseClpAmount(raw),activo:true}}).filter(Boolean);await AleAPI.post("savepricelist",{id:$("#plId").value||undefined,nombre:name,codigo:$("#plCode").value.trim(),descripcion:$("#plDescription").value.trim(),activo:$("#plActive").value,items},token);toast("✓ Lista de precios guardada");closePriceListEditor();await loadAdminModules({modules:["wholesale"],retry:true})}catch(err){console.warn(err);toast("✕ No fue posible guardar la lista")}}));
