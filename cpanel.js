...
const CPANEL_MEDIA_VERSION="20260916-r91812-high-demand-fix";
...
function renderOrderDetail(order,items,history=[]){
  ...
  const proofBox=$("#orderTransferProof"),proofLink=$("#orderTransferProofLink"),proofState=$("#orderTransferProofState"),approveBtn=$("#approveTransferPayment"),payWa=$("#sendOrderPaymentWhatsApp");if(proofBox){const transfer=String(order.medio_pago||"").toUpperCase()==="TRANSFERENCIA";proofBox.classList.toggle("hidden",!transfer);if(transfer){const has=!!order.comprobante_pago_url;proofLink.classList.toggle("hidden",!has);if(has)proofLink.href=order.comprobante_pago_url;proofState.textContent=has?(order.comprobante_pago_estado||"PENDIENTE_REVISION"):"Aún sin comprobante";approveBtn.classList.toggle("hidden",!has||String(order.estado_pago||"").toUpperCase()==="PAGADO")}}if(payWa)payWa.classList.toggle("hidden",String(order.medio_pago||"").toUpperCase()!=="TRANSBANK"||String(order.estado_pago||"").toUpperCase()==="PAGADO");
...
$("#sendOrderPaymentWhatsApp")?.addEventListener("click",async()=>{const o=data.orders.find(x=>String(x.id)===String(currentOrderDetailId));if(!o)return;const phone=String(o.telefono||"").replace(/\D/g,"");if(!phone)return toast("El pedido no tiene WhatsApp");try{const out=await AleAPI.post("adminorderpaymentlink",{id:o.id},token);const text=`Hola ${o.nombre||""}, tu pedido ${o.numero_pedido||o.id} está listo para pagar con Transbank. Abre este enlace seguro: ${out.payment_url}`;window.open(`https://wa.me/${phone}?text=${encodeURIComponent(text)}`,"_blank","noopener")}catch(err){console.warn(err);toast("No fue posible generar el enlace de pago")}});
$("#approveTransferPayment")?.addEventListener("click",async e=>busy(e.currentTarget,async()=>{try{await AleAPI.post("adminverifytransfer",{id:currentOrderDetailId},token);toast("✓ Transferencia verificada: pago PAGADO y pedido CONFIRMADO");await reload();if(currentOrderDetailId)openOrderDetail(currentOrderDetailId)}catch(err){console.warn(err);toast("✕ No fue posible confirmar la transferencia")}}));
