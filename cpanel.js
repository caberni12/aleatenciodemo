...
async function openProductStockEditor(id){const p=data.products.find(x=>String(x.id)===String(id));if(!p){toast("Producto no encontrado");return}const editor=$("#productStockEditor");if(!editor){console.error("PRODUCT_STOCK_EDITOR_MISSING");toast("No fue posible abrir Inventario. Actualiza el cPanel con Ctrl + F5.");return}$("#stockProductId").value=p.id;$("#productStockEditorTitle").textContent=`Inventario · ${p.nombre}`;const allow=stockPolicyAllowsNegative(p);$("#stockPolicySummary").innerHTML=allow?'<span class="stock-control-badge flexible">Flexible · permite negativo</span> La venta continúa aunque el stock sea 0 o negativo.':'<span class="stock-control-badge strict">Estricto · bloquea</span> La venta se detiene si el stock disponible no alcanza.';const sizes=(Array.isArray(p.tamanos)?p.tamanos:[]).filter(x=>String(x.activo??"SI").toUpperCase()!=="NO");$("#stockSizeId").innerHTML=sizes.length?sizes.map(z=>`<option value="${esc(z.id)}">${esc(z.nombre)} · stock ${Number(z.stock||0)}</option>`).join(""):'<option value="">Stock general</option>';$("#stockMovementType").value="INGRESO";$("#stockMovementValue").value="1";$("#stockMovementReference").value="";updateStockMovementUi();showEditor("productStockEditor",true);document.body.classList.add("product-stock-editor-open");requestAnimationFrame(()=>{editor.scrollTop=0;$("#stockMovementValue")?.focus({preventScroll:true})});await loadProductStockHistory(p.id)}
function closeProductStockEditor(){showEditor("productStockEditor",false);document.body.classList.remove("product-stock-editor-open")}
...
document.addEventListener("keydown",e=>{if(e.key==="Escape"&&!$("#productStockEditor")?.classList.contains("hidden"))closeProductStockEditor()});
...
function renderOrders(){
  ...
  host.innerHTML=table(["N.º pedido","Tipo","Fecha","Cliente / RUT","Contacto","Entrega","Total","Pago","Estado","PDF"],visibleOrders.map(o=>{
...
function openAdminView(view){const target=$(`.admin-nav button[data-view="${CSS.escape(String(view||"dashboard"))}"]`);if(!target)return;if(String(target.dataset.view)!=="wholesale-credits"&&typeof closeAllWholesaleCreditModals==="function")closeAllWholesaleCreditModals();$$('.admin-nav button').forEach(x=>x.classList.remove("active"));target.classList.add("active");$$('.admin-view').forEach(x=>x.classList.remove("active"));$("#view-"+target.dataset.view)?.classList.add("active");$("#viewTitle").textContent=target.textContent.trim();if(target.dataset.view==="products"){if($("#productSearch"))$("#productSearch").value="";if($("#productFilter"))$("#productFilter").value="";renderProducts()}if(target.dataset.view==="wholesale"){renderWholesale()}if(target.dataset.view==="wholesale-credits"){renderWholesaleCredits()}if(target.dataset.view==="suppliers"){renderSuppliers()}if(target.dataset.view==="inventory"){renderInventory()}if(target.dataset.view==="gallery"){renderGallery()}if(target.dataset.view==="reports"){loadReports(true).catch(err=>console.warn("reports open",err))}if(sidebarIsMobile())setSidebarOpen(false);window.scrollTo({top:0,behavior:"smooth"})}
...
const siiState={config:null,certificados:[],caf:[],documentos:[],loaded:false};
let siiLoadedOrderDocs=[];let siiLoadedOrderId=null;
const SII_DTE_NAMES={33:"Factura Electrónica",34:"Factura Exenta Electrónica",39:"Boleta Electrónica",41:"Boleta Exenta Electrónica",52:"Guía de Despacho Electrónica",56:"Nota de Débito Electrónica",61:"Nota de Crédito Electrónica"};
...
async function openSiiManagerModal(id){try{await loadSiiBilling(false)}catch(_){}openSiiModal(id)}
...
async function saveSiiConfig(){const payload={ambiente:$("#siiEnvironment").value,habilitado:$("#siiEnabled").value==="SI",rut_emisor:$("#siiRutEmisor").value.trim(),razon_social:$("#siiRazonSocial").value.trim(),giro:$("#siiGiro").value.trim(),acteco:$("#siiActeco").value.trim(),direccion_origen:$("#siiDireccion").value.trim(),comuna_origen:$("#siiComuna").value.trim(),ciudad_origen:$("#siiCiudad").value.trim(),direccion_regional_sii:$("#siiDireccionRegional")?.value.trim()||"",email_emisor:$("#siiEmail").value.trim(),numero_resolucion:$("#siiResolucionNumero").value,fecha_resolucion:$("#siiResolucionFecha").value};const out=await SiiAPI.saveConfig(payload,token);siiState.config=out.config||payload;renderSiiAll();toast("✓ Configuración SII guardada")}
...
function resolveSiiIssueMode(){const tipo=Number($("#siiTipoDte")?.value||0),requested=$("#siiIssueMode")?.value||"AUTO",prod=String(siiState.config?.ambiente||"").toUpperCase()==="PRODUCCION",official=siiAvailableCaf(tipo,false),local=siiAvailableCaf(tipo,true);if(prod)return{mode:"SII",official,local:null};if(requested==="PRUEBA_LOCAL")return{mode:local?"PRUEBA_LOCAL":"PRUEBA_LOCAL_SIN_FOLIOS",official,local};if(requested==="SII")return{mode:"SII",official,local};if(official)return{mode:"SII",official,local};if(local)return{mode:"PRUEBA_LOCAL",official,local};return{mode:"SII",official:null,local:null}}
function siiExistingForIssue(tipo=null,mode=null){
  ...
  const local=m.mode==="PRUEBA_LOCAL";return (siiLoadedOrderDocs||[]).find(x=>Number(x.tipo_dte)===t&&Boolean(x.es_prueba_local)===local)||null
}
const SINGLE_SII_ORDER_TYPES=new Set([33,34,39,41]);
function updateSiiIssueModeUi(){
  const btn=$("#siiEmitDte"),note=$("#siiIssueModeNotice"),sel=$("#siiIssueMode");if(!btn||!note)return;
  ...
  if(sel){sel.querySelector('option[value="PRUEBA_LOCAL"]')?.toggleAttribute('disabled',prod||!siiAvailableCaf(tipo,true));if(prod&&sel.value==="PRUEBA_LOCAL")sel.value="SII"}
  note.classList.toggle("is-local",m.mode.startsWith("PRUEBA_LOCAL"));
  if(existing){btn.innerHTML=`<i class="bi bi-eye"></i> Ver documento existente · Folio ${Number(existing.folio||0).toLocaleString('es-CL')}`;note.innerHTML=`<i class="bi bi-shield-lock"></i><span><strong>EMISIÓN PROTEGIDA:</strong> este pedido ya tiene ${esc(SII_DTE_NAMES[tipo]||`DTE ${tipo}`)} folio <b>${Number(existing.folio||0).toLocaleString('es-CL')}</b>. No se reservará ni consumirá otro folio. Para corregir una factura emitida corresponde usar Nota de Crédito/Débito según el caso.</span>`;return}
  if(m.mode==="PRUEBA_LOCAL"){btn.innerHTML='<i class="bi bi-file-earmark-pdf"></i> Generar PRUEBA LOCAL + PDF';note.innerHTML='<i class="bi bi-flask"></i><span><strong>PRUEBA LOCAL:</strong> se consumirá un folio mock, el XML se firmará con el PFX activo y se generará una representación PDF conforme a la guía de muestras impresas del SII. <b>No se enviará al SII.</b></span>'}
  else if(m.mode==="PRUEBA_LOCAL_SIN_FOLIOS"){btn.innerHTML='<i class="bi bi-exclamation-triangle"></i> Sin folios de prueba';note.innerHTML='<i class="bi bi-exclamation-triangle"></i><span>No hay folios PRUEBA LOCAL disponibles o vigentes para este tipo de DTE.</span>'}
  else if([39,41].includes(tipo)){btn.innerHTML='<i class="bi bi-cloud-slash"></i> API Boleta SII requerida';note.innerHTML='<i class="bi bi-info-circle"></i><span><strong>Boleta 39/41:</strong> el envío real utiliza la API REST específica de Boleta Electrónica del SII. Para pruebas internas selecciona <b>PRUEBA LOCAL</b>; tus folios 39/41 sí se pueden usar ahí sin enviarlos al SII.</span>'}
  else{btn.innerHTML='<i class="bi bi-receipt"></i> Firmar y enviar al SII';note.innerHTML=m.official?'<i class="bi bi-shield-check"></i><span><strong>CAF oficial disponible.</strong> La emisión usará el flujo real SII del ambiente seleccionado.</span>':'<i class="bi bi-exclamation-triangle"></i><span>No hay CAF oficial disponible para este tipo de documento. Carga un CAF SII o selecciona PRUEBA LOCAL.</span>'}
}
function toggleSiiReferenceFields(){const tipo=Number($("#siiTipoDte")?.value||0),show=[56,61].includes(tipo);$$('.sii-reference-field').forEach(x=>x.classList.toggle('hidden',!show));updateSiiIssueModeUi()}
...
async function generateSiiRepresentationPdf(rep,targetWindow=null){
  ...
  const em=rep.emisor||{},rc=rep.receptor||{},tot=rep.totales||{},items=Array.isArray(rep.items)?rep.items:[],pageW=215.9,margin=10;
  const logo=await imageUrlToDataUrl(em.logo_url||data.config?.logo_url||'logo-ale-atencio.png');
  ...
  const header=(copy)=>{
    ...
    doc.setDrawColor(190,0,0);doc.setTextColor(190,0,0);doc.setLineWidth(.7);doc.rect(126,9,80,38);doc.setFont('helvetica','bold');doc.setFontSize(11);doc.text(`R.U.T.: ${siiPdfText(em.rut)}`,166,18,{align:'center'});
    doc.setFontSize(10.5);const name=doc.splitTextToSize(siiPdfText(rep.nombre_dte||`DTE ${tipo}`).toUpperCase(),72);doc.text(name,166,26,{align:'center'});doc.setFontSize(12);doc.text(`N° ${Number(rep.folio||0).toLocaleString('es-CL')}`,166,41,{align:'center'});
    doc.setTextColor(0,0,0);doc.setDrawColor(0,0,0);doc.setFontSize(8);doc.text(`S.I.I. - ${siiPdfText(em.direccion_regional_sii||'DIRECCION REGIONAL')}`,166,52,{align:'center'});
  ...
  const receptor=()=>{
    ...
    doc.text('SEÑOR(ES):',13,67);doc.text('R.U.T.:',132,67);doc.text('GIRO:',13,75);doc.text('DIRECCIÓN:',13,83);doc.text('COMUNA:',132,83);doc.text('FECHA EMISIÓN:',132,91);
    doc.setFont('helvetica','normal');doc.text(siiPdfText(rc.razon_social),32,67,{maxWidth:94});doc.text(siiPdfText(rc.rut),147,67);doc.text(siiPdfText(rc.giro),26,75,{maxWidth:174});doc.text(siiPdfText(rc.direccion),34,83,{maxWidth:92});doc.text(`${siiPdfText(rc.comuna)}${rc.ciudad?` - ${siiPdfText(rc.ciudad)}`:''}`,150,83,{maxWidth:52});doc.text(siiPdfDate(rep.fecha_emision),160,91)
  };
  const tableHeader=(y)=>{doc.setFillColor(240,240,240);doc.rect(margin,y,pageW-2*margin,9,'F');doc.setDrawColor(165,165,165);doc.setFont('helvetica','bold');doc.setFontSize(7.7);doc.text('Item',12,y+5.8);doc.text('Código',24,y+5.8);doc.text('Descripción',48,y+5.8);doc.text('Cant.',139,y+5.8,{align:'right'});doc.text('Unid.',151,y+5.8);doc.text('P. Unitario',178,y+5.8,{align:'right'});doc.text('Valor',203,y+5.8,{align:'right'});doc.line(margin,y+9,pageW-margin,y+9);return y+9};
  const body=()=>{let y=101;y=tableHeader(y);doc.setFont('helvetica','normal');doc.setFontSize(7.8);for(const it of items){const desc=siiPdfText([it.nombre,it.descripcion].filter(Boolean).join(' - ')),lines=doc.splitTextToSize(desc,82),h=Math.max(8,lines.length*4+2);if(y+h>174)break;doc.text(String(it.nro||''),12,y+5.3);doc.text(siiPdfText(it.codigo||''),24,y+5.3,{maxWidth:20});doc.text(lines,48,y+5.3);doc.text(String(Number(it.cantidad||0)),139,y+5.3,{align:'right'});doc.text(siiPdfText(it.unidad||'UN'),151,y+5.3);doc.text(siiPdfMoney(it.precio_unitario),178,y+5.3,{align:'right'});doc.text(siiPdfMoney(it.monto_item),203,y+5.3,{align:'right'});doc.setDrawColor(226,226,226);doc.line(margin,y+h,pageW-margin,y+h);y+=h}return y};
  const drawTotals=()=>{let y=182;doc.setFont('helvetica','normal');doc.setFontSize(8.5);const isExempt=[34,41].includes(tipo);if(!isExempt){doc.text('MONTO NETO',145,y);doc.text(siiPdfMoney(tot.neto),203,y,{align:'right'});y+=6}if(Number(tot.exento||0)>0){doc.text('MONTO EXENTO',145,y);doc.text(siiPdfMoney(tot.exento),203,y,{align:'right'});y+=6}if(!isExempt&&Number(tot.iva||0)>0){doc.text(`IVA ${Number(tot.tasa_iva||19)}%`,145,y);doc.text(siiPdfMoney(tot.iva),203,y,{align:'right'});y+=6}doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text('MONTO TOTAL',145,y);doc.text(siiPdfMoney(tot.total),203,y,{align:'right'})};
  ...
  copies.forEach((copy,i)=>{if(i>0)doc.addPage('letter','portrait');header(copy);receptor();body();drawTotals();drawTimbre();drawVerificationQr();drawAcuse(copy);bottomNotice()});
...
async function loadSiiOrderData(){
  ...
  siiLoadedOrderId=String(o.id);siiLoadedOrderDocs=Array.isArray(out.documentos_existentes)?out.documentos_existentes:[];
...
async function emitSiiDte(){
  ...
  if([39,41].includes(tipo)&&mode.mode==="SII")return toast("Boleta 39/41: el envío REAL requiere la API REST específica de Boleta Electrónica del SII. Para probar ahora selecciona PRUEBA LOCAL y usa tus folios 39/41.");
  ...
  let pdfWindow=null;try{pdfWindow=window.open('about:blank','_blank');if(pdfWindow)pdfWindow.document.write('<title>Generando DTE</title><body style="font-family:Arial,sans-serif;padding:30px">Generando representación PDF del DTE…</body>');const out=await SiiAPI.issue(payload,token);
    ...
    if(out?.representacion){const pdf=await generateSiiRepresentationPdf(out.representacion,pdfWindow);await persistSiiPdf(out.documento_id,pdf)}else if(pdfWindow&&!pdfWindow.closed)pdfWindow.close();siiState.loaded=false;await loadSiiBilling(true);siiLoadedOrderDocs.unshift({id:out.documento_id,tipo_dte:out.tipo_dte,folio:out.folio,estado:out.estado,track_id:out.track_id,es_prueba_local:Boolean(out.es_prueba_local)});updateSiiIssueModeUi();try{if(typeof loadAdminModules==="function")await loadAdminModules({modules:["clients"],retry:false})}catch(e){console.warn("refresh clients after DTE",e)}const clientMsg=out?.cliente_creado?" · cliente creado en Clientes":out?.cliente_guardado?" · ficha de Cliente actualizada":"";if(local)toast(`✓ PRUEBA LOCAL generada · ${SII_DTE_NAMES[Number(out.tipo_dte)]||`DTE ${out.tipo_dte}`} · folio ${out.folio} · XML firmado + PDF abierto${clientMsg}`);else toast(`✓ DTE ${out.tipo_dte} folio ${out.folio}${out.track_id?` · TrackID ${out.track_id}`:''} · PDF generado${clientMsg}`);return out
...
$("#siiTestAuth")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const out=await SiiAPI.testAuth(token);toast(`✓ Autenticación SII OK · ${out.ambiente}`)}catch(err){toast(`✕ ${err.message||err}`)}}));
...
$("#siiTipoDte")?.addEventListener("change",toggleSiiReferenceFields);$("#siiIssueMode")?.addEventListener("change",updateSiiIssueModeUi);$("#siiPedidoId")?.addEventListener("input",()=>{siiLoadedOrderId=null;siiLoadedOrderDocs=[];updateSiiIssueModeUi()});toggleSiiReferenceFields();
