// R9.18.120-UI-MODALES-PRODUCCION
...
function renderWarehouseStock(){
  ...
  if($("#warehouseMovementsTable"))$("#warehouseMovementsTable").innerHTML=table(["Fecha","Bodega","Movimiento","Producto","Tamaño","Cantidad","Stock","Referencia"],(data.warehouseMovements||[]).slice(0,500).map(x=>`<tr><td>${esc(formatDate(x.creado_en))}</td><td>${esc(warehouseCode(x.bodega_id))}</td><td><span class="role-badge">${esc(x.tipo||"")}</span></td><td>${esc(productNameById(x.producto_id))}</td><td>${esc(productSizeById(x.producto_id,x.tamano_id)||"General")}</td><td class="${Number(x.cantidad||0)<0?"inventory-negative":"inventory-positive"}">${Number(x.cantidad||0)>0?"+":""}${Number(x.cantidad||0).toLocaleString("es-CL",{maximumFractionDigits:3})}</td><td>${Number(x.stock_anterior||0).toLocaleString("es-CL",{maximumFractionDigits:3})} → <strong>${Number(x.stock_nuevo||0).toLocaleString("es-CL",{maximumFractionDigits:3})}</strong></td><td>${esc(x.documento||x.referencia_id||x.motivo||"")}</td></tr>`).join(""));
}
...
async function consultLedgerDocument(value=""){const q=String(value||$("#ledgerSearch")?.value||"").trim();if(!q)return toast("Ingresa un documento, pedido o folio");try{const out=await AleAPI.post("ledgerdocumenthistory",{q},token);const order=out.order||{},dtes=out.dtes||[],ops=out.operations||[];$("#ledgerHistoryTitle").textContent=order.numero_pedido||dtes[0]?`${order.numero_pedido||`DTE ${dtes[0]?.tipo_dte}-${dtes[0]?.folio}`}`:q;$("#ledgerDocumentSummary").innerHTML=`<div><span>Pedido</span><strong>${esc(order.numero_pedido||order.id||"—")}</strong></div><div><span>Cliente</span><strong>${esc(order.nombre||"—")}</strong></div><div><span>Total</span><strong>${order.total!==undefined?money(order.total):"—"}</strong></div><div><span>Bodega</span><strong>${esc(order.bodega_codigo_snapshot||warehouseCode(order.bodega_id)||"—")}</strong></div>${dtes.length?`<div class="span-2"><span>DTE</span><strong>${dtes.map(x=>`${x.tipo_dte}-${x.folio}`).join(" · ")}</strong></div>`:""}${ops.length?`<div class="span-2"><span>Inventario</span><strong>${ops.map(x=>x.numero).join(" · ")}</strong></div>`:""}`;$("#ledgerTimeline").innerHTML=(out.timeline||[]).map(x=>`<div class="ledger-timeline-row"><span class="ledger-dot"></span><div><strong>${esc(x.titulo||x.tipo)}</strong><small>${esc(formatDate(x.fecha))}${x.detalle?` · ${esc(x.detalle)}`:""}${Number(x.monto||0)?` · ${money(x.monto)}`:""}</small></div></div>`).join("")||'<div class="empty-card">Sin historial relacionado.</div>';showEditor("ledgerHistoryEditor",true)}catch(err){console.warn(err);toast("✕ No fue posible encontrar el historial")}}
$("#ledgerConsult")?.addEventListener("click",()=>consultLedgerDocument());$("#ledgerSearch")?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();consultLedgerDocument()}});
$("#ledgerTable")?.addEventListener("dblclick",e=>{const row=e.target.closest("[data-ledger-doc]");if(row?.dataset.ledgerDoc)consultLedgerDocument(row.dataset.ledgerDoc)});
...
function openAdminView(view){const target=$(`.admin-nav button[data-view="${CSS.escape(String(view||"dashboard"))}"]`);if(!target)return;if(String(target.dataset.view)!=="wholesale-credits"&&typeof closeAllWholesaleCreditModals==="function")closeAllWholesaleCreditModals();$$('.admin-nav button').forEach(x=>x.classList.remove("active"));target.classList.add("active");$$('.admin-view').forEach(x=>x.classList.remove("active"));$("#view-"+target.dataset.view)?.classList.add("active");$("#viewTitle").textContent=target.textContent.trim();if(target.dataset.view==="products"){if($("#productSearch"))$("#productSearch").value="";if($("#productFilter"))$("#productFilter").value="";renderProducts()}if(target.dataset.view==="wholesale"){renderWholesale()}if(target.dataset.view==="wholesale-credits"){renderWholesaleCredits()}if(target.dataset.view==="suppliers"){renderSuppliers()}if(target.dataset.view==="warehouses"){renderWarehouses()}if(target.dataset.view==="stock"){renderWarehouseStock()}if(target.dataset.view==="ledger"){renderLedger()}if(target.dataset.view==="inventory"){renderInventory()}if(target.dataset.view==="gallery"){renderGallery()}if(target.dataset.view==="reports"){loadReports(true).catch(err=>console.warn("reports open",err))}if(sidebarIsMobile())setSidebarOpen(false);window.scrollTo({top:0,behavior:"smooth"})}
...
const SII_DTE_NAMES={33:"Factura Electrónica",34:"Factura No Afecta o Exenta Electrónica",39:"Boleta Electrónica",41:"Boleta No Afecta o Exenta Electrónica",52:"Guía de Despacho Electrónica",56:"Nota de Débito Electrónica",61:"Nota de Crédito Electrónica"};
