...
function renderPayments(){const c=data.config||{},checkout=c.transbank_checkout_url||TRANSBANK_DEFAULT_STOREFRONT_URL,returnUrl=c.transbank_return_url||checkout||TRANSBANK_DEFAULT_STOREFRONT_URL;if($("#pTransbankCheckoutUrl"))$("#pTransbankCheckoutUrl").value=checkout;if($("#pTransbankReturnUrl"))$("#pTransbankReturnUrl").value=returnUrl;refreshTransbankHealth(false)}
$("#savePayments")?.addEventListener("click",e=>busy(e.currentTarget,async()=>{try{const checkoutRaw=$("#pTransbankCheckoutUrl")?.value.trim()||"",returnRaw=$("#pTransbankReturnUrl")?.value.trim()||"";const checkout=checkoutRaw?safeHttpsAdminUrl(checkoutRaw):"",returnUrl=returnRaw?safeHttpsAdminUrl(returnRaw):"";if(!checkout||!returnUrl){toast("✕ Debes indicar ambos dominios HTTPS");return}await AleAPI.post("saveConfig",{transbank_enabled:"SI",transbank_checkout_url:checkout,transbank_return_url:returnUrl,transbank_button_label:"Pagar con Transbank"},token);toast("✓ Dominios Transbank guardados. Verificando…");await reload();await refreshTransbankHealth(false)}catch(err){console.warn(err);const code=String(err?.message||"");toast(code.includes("TRANSBANK_URL")||code.includes("URL_HTTPS")?"✕ Ambos dominios deben ser HTTPS públicos y no pueden ser una URL de Supabase":"✕ No fue posible guardar Transbank")}}));
