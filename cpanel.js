...
async function generateAssignedPaymentLink(mode,btn){
  ...
  const label=new Date(out.expires_at||expiresAt).toLocaleString("es-CL");const text=`Hola ${o.nombre||""}, tu pedido ${o.numero_pedido||o.id} está listo para pagar con Transbank. Este enlace estará disponible hasta ${label}: ${out.payment_url}`;
  ...
  else{await navigator.clipboard.writeText(out.payment_url);toast(`✓ Enlace copiado · vigente hasta ${label}`)}
}
