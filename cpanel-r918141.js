...
function openMovementDetail(key){
  ...
  $("#movementDetailSubtitle").textContent=scope==="inventory"?"Detalle completo del movimiento de stock seleccionado.":"Detalle contable y documental del movimiento seleccionado.";
  let parts=[];
  if(scope==="inventory"){
    ...
    parts=[
      ...
      movementDetailItem("Folio",x.folio_operacion||inventoryOperationById(x.operacion_id)?.numero||"—"),
      movementDetailItem("Documento",x.documento||"—","wide"),
      movementDetailItem("Referencia",x.referencia_id||"—","wide")
    ];
    const note=[x.motivo?`Motivo: ${x.motivo}`:"",x.operacion_id?`Operación: ${x.operacion_id}`:"",x.id?`Movimiento: ${x.id}`:""].filter(Boolean).join(" · ");
