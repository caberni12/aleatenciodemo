...
const MEDIA_VERSION = "20260916-r9170-transbank-integraciones";
...
async function sendAndConfirm(action, type, data){
  ...
  try{
    ...
    if(r?.ok!==false) return {ok:true,id:r?.id||data.id,numero_solicitud:r?.numero_solicitud||"",numero_pedido:r?.numero_pedido||"",pdf_url:r?.pdf_url||"",pdf_pending:!!r?.pdf_pending,total:r?.total,source:"transport",persisted:r?.persisted!==false};
  ...
  for(let i=0;i<7;i++){
    ...
    try{
      ...
      if(check?.ok&&check?.exists) return {ok:true,id:data.id,numero_solicitud:check?.numero_solicitud||"",numero_pedido:check?.numero_pedido||"",pdf_url:check?.pdf_url||"",source:"verified-after-transport"};
    }catch(_){ }
  }
...
function transbankPaymentUrl(){return safeHttpsUrl(state.config?.transbank_payment_url)}
function transbankAvailable(){return configYes(state.config?.transbank_enabled)&&!!transbankPaymentUrl()}
...
function syncPaymentUI(){
  ...
  const success=$("#orderSuccessTransbank");if(success){success.innerHTML=`<i class="bi bi-credit-card-2-front"></i> ${esc(transbankButtonLabel())}`;success.href=transbankPaymentUrl()||"#";success.hidden=!available}
}
...
async function submitOrder(){
  ...
  if(saved){
    ...
    if(payWithTransbank){
      try{localStorage.setItem("aleAtencioLastOrder",JSON.stringify({numero_pedido:orderId,total:t.total,provider:"TRANSBANK",saved_at:new Date().toISOString()}))}catch(_){ }
      const payUrl=transbankPaymentUrl();
      setCheckoutPaymentIntent("");
      endButtonLoader(btn);
      toast(`Pedido ${orderId} registrado. Abriendo Transbank…`,"success");
      setTimeout(()=>window.location.assign(payUrl),450);
      return;
    }
    setCheckoutPaymentIntent("");
    try{
      if(normalizePhone(state.config.whatsapp)) openWhatsApp(`Hola Ale Atencio, quiero confirmar mi pedido ${orderId}.\n\n${lines}\n\nTotal: ${money(t.total)}\nNombre: ${nombre}\nRUT: ${rut}\nEntrega: ${data.metodo_entrega}\nDirección: ${data.direccion}\nObservaciones: ${data.observaciones}${result?.pdf_url?`\nRespaldo PDF: ${result.pdf_url}`:""}`);
